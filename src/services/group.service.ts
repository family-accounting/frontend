...
export const useGroupService = () => {
  ...
  const createGroup = async (group: Pick<IGroup, 'name' | 'icon' | 'color' | 'description'>) => {
    const response = await api.post('/groups', group);
    return response.data;
  };

  const updateGroup = async (id: IGroup['id'], group: Partial<Pick<IGroup, 'name' | 'icon' | 'color' | 'description'>   >) => {
